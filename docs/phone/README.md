...
我们这里需要注意的是 `vw`、`vh`、`vmain`、`vmax`他们的百分比是相对于视口的。而这个视口的大小我们可以通过`viewport`来设置。比如将`viewport`的`width`属性设置成`200`,那么`1vw` 就相当于`2px`，关于`viewport`下面会讲到。另外要区分css中的百分比，百分比是相对与父元素的百分比的。
